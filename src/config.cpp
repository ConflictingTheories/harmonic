...
{
    // Already initialized with defaults in header
}
...
std::string Config::get_mode_string() const
{
    switch (mode)
    {
...
void Config::parse_line(const std::string &line)
{
    ...
    if (key == "mode")
    {
        if (value == "radio")
            ...
            mode = PlaybackMode::DJ;
        else if (value == "coder")
            mode = PlaybackMode::CODER;
    }
    else if (key == "theme")
    {
        if (value == "cyberpunk")
            theme = VisualizerTheme::CYBERPUNK_COFFEE;
        else if (value == "forest")
            theme = VisualizerTheme::PIXEL_FOREST;
        else if (value == "netherworld")
            theme = VisualizerTheme::DEMONIC_NETHERWORLD;
    }
