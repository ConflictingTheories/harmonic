...
## Main Tasks
- [x] Add libshout and websocketpp dependencies to CMakeLists.txt
- [x] Update config.h to include streaming server configuration
- [x] Modify network_server.h to support libshout and WebSockets
- [x] Implement libshout streaming in network_server.cpp
- [x] Implement WebSocket handling for FFT in network_server.cpp
- [x] Update templates/index.html to use WebSockets for FFT data
- [ ] Test audio streaming and FFT WebSocket updates
- [ ] Perform code review and suggest improvements

## Additional TODOs (After Main Tasks)
- Add "Admin" mode (Remote Web Controller with Permissions) and remote control API (only for admin)
- Add additional filter effects (like colours, gradients, etc) on frontend app.
- Improve metadata - Have a small info tooltip with additional information on artist, track, album, if present.
- Fix up audio streaming quality and latency.
...
- On frontend - dim the "colours" of the theme when 'paused' and them bring them back to saturation on play (intensity should be based on volume level)
